...
class HomeController extends Controller
{
    ...
    public function getIndex() {
        ...
        $index_cat = BusinessCategory::query()->where('parent_id', 0)->limit(12)->get();
    ...
    public function allFreelancers(){
        ...
        $freelancers = User::orderBy('id','DESC')->where('role','Artisan')->where('is_admin',0)->where('status', 1)->paginate(18);
    ...
    public function allServices(){
        ...
        $artisan_services = ArtisanServices::query()->orderBy('id','DESC')->paginate(18);

        foreach ($artisan_services as $row){
            $row["profile_image"] = User::query()->where('id', $row->user_id)->value('profile_image');
            $row["full_name"] = User::query()->where('id', $row->user_id)->value('full_name');
    ...
    public function getUserDetails(Request $request){
        ...
        $reviews =  ServiceRating::query()->where('type',"freelancer")->where('user_id',$user[0]->id)->get();
        $skills = MySkill::query()->where('user_id',$user[0]->id)->get();
        ...
        $listed = ['user' => $user, 'awards' =>$awards, 'education' => $education,
            'experience' => $experience, 'reviews' => $reviews,'gallery' =>$gallery,'status' =>$status, 'services' =>$services, 'skills' => $skills];
        if (App::environment('production')) {
            $listed = array_merge($listed,$seo);
        }
